...
traditionally designed audio plugins. ChowMatrix is made
up a ``tree'' of delay nodes, where each node has a ``parent''
...
``lock'' the parameter so that it will not be affected by
the ``Insanity'' control. \shortcut{CMD+Click} on the mod

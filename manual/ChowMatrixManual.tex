...
delayed signal is fed back to the input. Higher feedback
values will cause the delay node to "echo" more times.
\newpar
By setting the feedback to control to its maximum value,
the node enters \boldtheme{Freeze} mode, which freezes the
...
\begin{itemize}
    \item \hyperlink{goto:add-node}{Add a node}
    \item \hyperlink{goto:delete-node}{Delete a node}
    \item \hyperlink{goto:unison}{Change parameters for all nodes}
\end{itemize}
